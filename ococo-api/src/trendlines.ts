...
import { publishSignal, publishSignalRemove } from './signalBus.js'
...
interface ChannelCandidate {
  support: TrendlineCandidate
  resistance: TrendlineCandidate
...
function detectPivots(bars: Bar[], lookback: number, source: 'wick' | 'body'): Pivot[] {
  ...
  for (let i = lookback; i < len - lookback; i++) {
    ...
    for (let j = i - lookback; j <= i + lookback; j++) {
      ...
      if (cmp >= highVal) { isHigh = false; break }
    }
    if (isHigh) {
    ...
    for (let j = i - lookback; j <= i + lookback; j++) {
      ...
      if (cmp <= lowVal) { isLow = false; break }
    }
    if (isLow) {
...
function priceAtBar(slope: number, intercept: number, index: number): number {
  return slope * index + intercept
}

function scoreTrendline(
  candidate: { p1: Pivot; p2: Pivot; slope: number; intercept: number },
  bars: Bar[],
  allPivots: Pivot[],
  direction: 'support' | 'resistance',
): TrendlineCandidate {
  ...
  for (const piv of allPivots) {
    if (piv.index === p1.index || piv.index === p2.index) continue
    if (piv.index < p1.index || piv.index > p2.index) continue
    const expected = priceAtBar(slope, intercept, piv.index)
    if (Math.abs(piv.price - expected) < tolerance) touches++
  }
  ...
  for (let i = p1.index; i <= p2.index; i++) {
    ...
    if (direction === 'support') {
      if (bars[i].low < expected - tolerance) breaks++
    } else {
      if (bars[i].high > expected + tolerance) breaks++
    }
  }
...
function detectTrendlines(bars: Bar[], lookback: number, source: 'wick' | 'body', maxCandidates = 200): TrendlineCandidate[] {
  ...
  for (let i = 0; i < lows.length && candidates.length < maxCandidates; i++) {
    for (let j = i + 1; j < lows.length && candidates.length < maxCandidates; j++) {
      ...
      if (p2.index - p1.index < lookback) continue // too close
  ...
  for (let i = 0; i < highs.length && candidates.length < maxCandidates * 2; i++) {
    for (let j = i + 1; j < highs.length && candidates.length < maxCandidates * 2; j++) {
      ...
      if (p2.index - p1.index < lookback) continue
      const slope = (p2.price - p1.price) / (p2.index - p1.index)
      const intercept = p1.price - slope * p1.index
...
function detectChannels(trendlines: TrendlineCandidate[]): ChannelCandidate[] {
...
const SOURCES: Array<'wick' | 'body'> = ['wick', 'body']
    ...
    for (const source of SOURCES) {
      for (const lookback of config.lookbacks) {
        ...
        for (const tl of trendlines) {
          if (tl.score < 3) continue // skip weak lines
...
function deduplicateTrendlines(annotations: Annotation[]): Annotation[] {
  const result: Annotation[] = []
  for (const ann of annotations) {
    const isDupe = result.some(existing => {
      if (existing.type !== ann.type) return false
      if (existing.points.length !== ann.points.length) return false
