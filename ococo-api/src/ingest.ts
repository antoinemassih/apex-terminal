...
async function fetchBars(symbol: string, interval: string, period: string): Promise<Bar[]> {
  try {
    ...
    if (!resp.ok) return []
    return resp.json() as Promise<Bar[]>
  } catch {
    return []
  }
}
...
async function ingestSymbol(symbol: string): Promise<Record<string, number>> {
...
export async function ingestSingle(symbol: string): Promise<Record<string, number>> {
  const counts = await ingestSymbol(symbol)
