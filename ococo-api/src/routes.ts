...
export async function registerRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Querystring: { symbol?: string; interval?: string; start?: string; before?: string; limit?: string } }>('/api/bars', async (req, reply) => {
  ...
  app.get<{ Querystring: { symbol?: string; interval?: string } }>('/api/bars/count', async (req, reply) => {
    ...
    if (redisCount > 0) return { count: redisCount, source: 'redis' }
    const influxCount = await barCount(symbol, interval)
    return { count: influxCount, source: 'influx' }
  })

  app.get<{ Querystring: { symbol?: string; interval?: string } }>('/api/bars/range', async (req, reply) => {
    const { symbol, interval } = req.query
    if (!symbol || !interval) return reply.code(400).send({ error: 'symbol and interval required' })
