...
export async function searchSymbols(q, limit = 20) {
    ...
    const result = await query(`SELECT symbol, name, type, exchange, sector FROM symbols
     ...
     ORDER BY
       CASE WHEN symbol = $3 THEN 0
            ...
            ELSE 2 END,
       symbol
     LIMIT $5`, [`%${upper}%`, `%${q}%`, upper, `${upper}%`, limit]);
    return result.rows;
}
/** Get all symbols (for browsing) */
export async function listSymbols(type) {
    if (type) {
...
export async function upsertSymbol(info) {
    await query(`INSERT INTO symbols (symbol, name, type, exchange, sector, updated_at)
     ...
     ON CONFLICT (symbol) DO UPDATE SET
       ...
       updated_at = NOW()`, [info.symbol.toUpperCase(), info.name, info.type, info.exchange, info.sector]);
}
/** Get recent symbols for a session */
export async function getRecents(sessionId = 'default', limit = 20) {
    const result = await query(`SELECT r.symbol, s.name, r.accessed_at
...
export async function touchRecent(symbol, sessionId = 'default') {
    await query(`INSERT INTO recent_symbols (session_id, symbol, accessed_at)
     VALUES ($1, $2, NOW())
     ON CONFLICT (session_id, symbol)
     DO UPDATE SET accessed_at = NOW()`, [sessionId, symbol.toUpperCase()]);
    // Also ensure the symbol exists in the catalog (auto-add unknown symbols)
    await query(`INSERT INTO symbols (symbol, type) VALUES ($1, 'unknown') ON CONFLICT DO NOTHING`, [symbol.toUpperCase()]);
}
