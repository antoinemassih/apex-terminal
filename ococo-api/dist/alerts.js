...
export async function listAlerts(symbol) {
    if (symbol) {
...
export async function getActiveAlerts(symbol) {
    const result = await query('SELECT * FROM alert_rules WHERE symbol = $1 AND active = true', [symbol]);
    return result.rows.map(rowToAlert);
}
export async function createAlert(alert) {
    const result = await query(`INSERT INTO alert_rules (annotation_id, symbol, condition, price, active, cooldown_sec, notify)
...
export async function updateAlert(id, updates) {
    ...
    if (updates.cooldown_sec !== undefined) {
        sets.push(`cooldown_sec = $${idx++}`);
        params.push(updates.cooldown_sec);
    }
    if (updates.notify !== undefined) {
        sets.push(`notify = $${idx++}`);
        params.push(JSON.stringify(updates.notify));
    }
    if (sets.length === 0)
        return null;
...
export async function checkAlerts(symbol, price) {
    ...
    for (const alert of alerts) {
        ...
        if (alert.last_triggered) {
            ...
            if (elapsed < alert.cooldown_sec * 1000)
                continue;
        }
        let hit = false;
