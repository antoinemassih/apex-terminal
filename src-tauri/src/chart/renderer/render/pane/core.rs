...
use crate::chart_renderer::ui::style::{
    ...
    mono_at, prop_at, prop_3xs, prop_sm, prop_lg, prop_xl,
    contrast_fg, color_shade, link_group_palette,
    font_2xs, font_xs, font_sm, font_xl,
};
...
fn render_chart_pane(
        ...
        if let Some(dot_rect) = hdr.link_dot_rect {
            ...
            fn draw_swatch(
                ...
                match opt {
                    ...
                    LinkOpt::New => { ui.painter().text(
                        ...
                        prop_at(dot_r * 2.0), theme.text()); }
                    LinkOpt::Group { color, gid, .. } => {
                        ui.painter().circle_filled(center, dot_r, *color);
                        // This was a verbatim re-implementation of
                        // `style::contrast_fg` — same 0.299/0.587/0.114
                        // weights, same 140.0 threshold.
                        let txt_col = contrast_fg(*color);
                        ui.painter().text(
                            center, egui::Align2::CENTER_CENTER, format!("{}", gid),
                            prop_at(dot_r * 1.3), txt_col,
                        );
                    }
                }
            }
            ...
            if res.changed {
                if sel_idx == new_group_sentinel {
                    // Identity ramp for user-created link groups; its first
                    // four rungs are `drawing_palette()`. See
                    // `style::link_group_palette`.
                    let new_group_colors = link_group_palette();
            ...
            .show(ui.ctx(), |ui| {
                ...
                ui.horizontal(|ui| {
                    for (ptype, label) in [
                        ...
                        if ui.add(egui::Button::new(
                            TextStyle::BodySm.as_rich_cascading(label, fg))
                            .fill(bg)
                            .corner_radius(4.0)
                        ).clicked() {
                ...
                match chart.pane_type {
                    PaneType::Chart => {
                        ...
                        ui.horizontal(|ui| {
                            ui.label(TextStyle::BodySm.as_rich_cascading("Template:", t.dim));
                            ...
                            let stock_btn = ui.add(egui::Button::new(
                                TextStyle::BodySm.as_rich_cascading("Stock", if stock_active { t.accent } else { t.dim }))
                            ...
                            let opt_btn = ui.add(egui::Button::new(
                                TextStyle::BodySm.as_rich_cascading("Option", if opt_active { t.accent } else { t.dim }))
                        ...
                        if !chart.pane_picker_option_mode {
                            ...
                            if chart.pane_picker_query.is_empty() {
                                ...
                                for (sym, name) in &recents {
                                    ui.horizontal(|ui| {
                                        if ui.add(egui::Button::new(
                                            TextStyle::BodySm.as_rich_cascading(sym, t.text))
                                        ...
                                        ui.label(TextStyle::BodySm.as_rich_cascading(name, t.dim));
                                    });
                                }
                            } else {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                if ui.add(egui::Button::new(TextStyle::BodyLg.as_rich_cascading(crate::ui_kit::icons::Icon::CARET_LEFT, back_col))
                                    .fill(egui::Color32::TRANSPARENT)
                                ).clicked() && can_back {
                                ...
                                if ui.add(egui::Button::new(TextStyle::BodyLg.as_rich_cascading(crate::ui_kit::icons::Icon::CARET_RIGHT, fwd_col))
                                    .fill(egui::Color32::TRANSPARENT)
                                ).clicked() && can_fwd {
                                    chart.option_quick.dte_idx = dte_idx + 1;
                                    let new_dte = DTE_LIST[dte_idx + 1];
    ...
    if last_price > 0.0 {
        ...
        if price_y.is_finite() && price_y >= rect.top() + pt && price_y <= rect.top() + pt + ch {
            ...
            if watchlist.show_y_axis {
                ...
                let fg_col = color_shade(price_col, 0.15, 255);
    ...
    if chart.show_volume && !chart.bar_source_mark {
        ...
        if chart.show_delta_volume && chart.delta_data.len() == n {
            ...
            for i in (vs as u32)..end { if let Some(b) = chart.bars.get(i as usize) {
                ...
                let bar_color = color_alpha(base_color, alpha);
        ...
        if let Some(ref vp) = chart.vp.data {
            ...
            for level in &vp.levels {
                ...
                if level.total_vol > avg_vol * 1.5 {
                    ...
                    painter.circle_filled(egui::pos2(rect.left()+cw+2.0, y), 1.5, color_alpha(t.dim, 60));
                }
            }
        }
    }
        ...
        match chart.candle_mode {
            ...
            CandleMode::Violin | CandleMode::ViolinGradient => {
                ...
                for (level_price, width_frac, buy_ratio) in &micro {
                    ...
                    let color = if chart.candle_mode == CandleMode::ViolinGradient {
                        ...
                        color_shade(base_color, brightness, alpha)
                    } else {
                        color_alpha(base_color, alpha)
                    };
            ...
            CandleMode::Gradient => {
                ...
                for si in 0..num_slices {
                    ...
                    let color = color_shade(base_color, brightness, 220);
    ...
    if let Some(overlay) = chart.replay_overlay.as_ref() {
        if !overlay.bars.is_empty() && !chart.timestamps.is_empty() {
            ...
            let body_c = color_alpha(base, 160);
            let wick_c = color_alpha(base, 220);
            ...
            // Was a frozen 11.0; nearest rung is font_sm() (12.0) — +1px.
            let font_id = prop_sm();
        ...
        if main_base > 0.0 && overlay_base > 0.0 {
            ...
            if ov.show_candles {
                ...
                for i in start_idx..end_idx {
                    if let Some(ob) = ov.bars.get(i) {
                        ...
                        let col = color_alpha(overlay_color, alpha);
            ...
            if let Some(ob) = ov.bars.get(badge_bar) {
                ...
                if badge_y.is_finite() {
                    ...
                    painter.rect_filled(br, 3.0, color_alpha(overlay_color, 30));
    ...
    // ── Strikes overlay button — moved to pane header right cluster.
    //    See painter_pane.rs OPTIONS button + click handler near hdr.clicked_options.
    //    Loading spinner still requested below for chain fetches.
    if chart.show_strikes_overlay && chart.overlay_chain_loading {
        ctx.request_repaint();
    }
    ...
    if chart.show_strikes_overlay {
        ...
        if (!calls.is_empty() || !puts.is_empty()) && last_price > 0.0 {
            ...
            for si in &strikes {
                ...
                let pill_bg = color_alpha(base_col, pill_alpha);
                ...
                if is_hovered {
                    ...
                    painter.text(tag_rect.center(), egui::Align2::CENTER_CENTER,
                        &format!("{:.0}", si.strike), mono_xs(), contrast_fg(base_col));
                    ...
                    painter.text(chart_btn_rect.center(), egui::Align2::CENTER_CENTER, Icon::CHART_LINE, prop_sm(), t.accent);
        ...
        for order in ib_orders {
            ...
            if is_buy {
                ...
                painter.add(egui::Shape::convex_polygon(
                    ...
                    color, egui::Stroke::new(0.5, contrast_fg(color))));
            } else {
    ...
    if chart.swing_leg_mode > 0 && n > 20 {
        ...
        if let Some(((pivot_i, pivot_price), is_peak)) = most_recent {
            ...
            let col = if from_above {
                color_alpha(t.bear, 120)
            } else {
                color_alpha(t.bull, 120)
            };
            ...
            if chart.swing_leg_mode == 1 {
                ...
                painter.rect_filled(lr, 4.0, color_alpha(t.bg, 220));
    ...
    if !chart.hide_all_indicators {
        for ind in &chart.indicators {
            ...
            let dim_color = color_alpha(base_rgb, 120);
            let fill_color = color_alpha(base_rgb, 18);
            ...
            if let Some(&last_val) = ind.values.iter().rev().find(|v| !v.is_nan()) {
                ...
                if label_y.is_finite() && label_y > rect.top() + pt && label_y < rect.top() + pt + ch {
                    ...
                    painter.rect_filled(bg, 2.0, color_alpha(t.toolbar_bg, 200));
    ...
    for d in &chart.drawings {
        ...
        match &d.kind {
            ...
            DrawingKind::BarMarker{time,price,up}=>{
                ...
                if is_sel {
                    painter.circle_stroke(egui::pos2(x, y), 8.0, egui::Stroke::new(1.5, t.text));
                }
            }
            ...
            DrawingKind::Channel{price0,time0,price1,time1,offset} | DrawingKind::FibChannel{price0,time0,price1,time1,offset}=>{
                ...
                if p0.x.is_finite() && p1.x.is_finite() && p0.y.is_finite() && p1.y.is_finite() {
                    ...
                    if is_sel {
                        ...
                        painter.circle_stroke(p0, 5.0, egui::Stroke::new(1.0, contrast_fg(COLOR_INFO_CYAN)));
                        painter.circle_filled(p1, 5.0, COLOR_INFO_CYAN);
                        painter.circle_stroke(p1, 5.0, egui::Stroke::new(1.0, contrast_fg(COLOR_INFO_CYAN)));
            ...
            DrawingKind::Pitchfork{price0,time0,price1,time1,price2,time2} => {
                ...
                if use_gpu_drawing {
                    #[cfg(feature = "gpu_chart_v2")]
                    {
                        ...
                        // Median runs from anchor p0 through the p1/p2 midpoint.
                        let mid_bar = (bar1g + bar2g) * 0.5;
                        let mid_price = (*price1 + *price2) * 0.5;
                        let mid_slot = mid_bar - vs_floor;
                        let mslope = if (mid_slot - s0).abs() > 0.001 {
                            (mid_price - *price0) / (mid_slot - s0)
                        } else { 0.0 };
                        // Value of a line with the median slope through (anchor_s, anchor_v)
                        // evaluated at the chart's left/right slot edges.
                        let line_at = |anchor_s: f32, anchor_v: f32| -> (f32, f32) {
                            (anchor_v + mslope * (edge_left_slot  - anchor_s),
                             anchor_v + mslope * (edge_right_slot - anchor_s))
                        };
                        let (med_l, med_r) = line_at(s0, *price0);
                        let (up_l,  up_r)  = line_at(s1, *price1);
                        let (lo_l,  lo_r)  = line_at(s2, *price2);
                        // Parallelogram fill between upper and lower parallels.
                        chart.gpu_render_params.fill_quads.push(FillQuad {
                            start_slot: edge_left_slot,  start_top: up_l.max(lo_l), start_bot: up_l.min(lo_l),
                            end_slot:   edge_right_slot, end_top:   up_r.max(lo_r), end_bot:   up_r.min(lo_r),
                            color: _drawing_c32(hex_to_color(&d.color, d.opacity * 0.05)),
                        });
                        let (dp, dd) = match d.line_style {
                            LineStyle::Solid  => LineSegment::SOLID,
                            LineStyle::Dashed => LineSegment::DASHED,
                            LineStyle::Dotted => LineSegment::DOTTED,
                        };
                        let push = |segs: &mut Vec<LineSegment>, l: f32, r: f32, col: egui::Color32, th: f32, pdp: f32, pdd: f32| {
                            segs.push(LineSegment {
                                start_slot: edge_left_slot,  start_val: l,
                                end_slot:   edge_right_slot, end_val:   r,
                                color: _drawing_c32(col), thickness: th * drawing_ppp,
                                dash_period_px: pdp, dash_duty: pdd,
                            });
                        };
                        let segs = &mut chart.gpu_render_params.line_segments;
                        // Median + the two parallels (user line style).
                        push(segs, med_l, med_r, if is_sel { t.accent } else { dc }, d.thickness, dp, dd);
                        push(segs, up_l, up_r, color_alpha(dc, 180), d.thickness * 0.8, dp, dd);
                        push(segs, lo_l, lo_r, color_alpha(dc, 180), d.thickness * 0.8, dp, dd);
                        // Half-way dotted lines (median↔parallel midpoints).
                        let (hdp, hdd) = LineSegment::DOTTED;
                        push(segs, (med_l + up_l) * 0.5, (med_r + up_r) * 0.5, color_alpha(dc, 80), d.thickness * 0.4, hdp, hdd);
                        push(segs, (med_l + lo_l) * 0.5, (med_r + lo_r) * 0.5, color_alpha(dc, 80), d.thickness * 0.4, hdp, hdd);
                        // Connector p1→p2 (solid, short).
                        segs.push(LineSegment {
                            start_slot: s1, start_val: *price1,
                            end_slot:   s2, end_val:   *price2,
                            color: _drawing_c32(color_alpha(dc, 100)),
                            thickness: d.thickness * 0.5 * drawing_ppp,
                            dash_period_px: 0.0, dash_duty: 1.0,
                        });
                    }
                } else {
                ...
                if is_sel {
                    for &pt in &[p0, p1, p2] {
                        ...
                        painter.circle_stroke(pt, 5.0, egui::Stroke::new(1.0, contrast_fg(COLOR_INFO_CYAN)));
                    }
                }
            }
            DrawingKind::GannFan{price0,time0,price1,time1} => {
                ...
                // Slot/value reference for the GPU path. The fan's price-per-slot
                // slope at ratio R is `pv_slope * R` where pv_slope is the
                // reference line's price delta over slot delta. Rendered through
                // the shared bx()/py() transform this reproduces the egui fan's
                // screen angles exactly (both use the same pixels-per-unit).
                #[cfg(feature = "gpu_chart_v2")]
                let gann_slot_val = {
                    let bar0g = SignalDrawing::time_to_bar(*time0, &chart.timestamps);
                    let bar1g = SignalDrawing::time_to_bar(*time1, &chart.timestamps);
                    ...
                    let s1 = bar1g - vs_floor;
                    let dslot = s1 - s0;
                    let pv_slope = if dslot.abs() > 0.001 { (*price1 - *price0) / dslot } else { 0.0 };
                    (s0, pv_slope)
                };
                for &(ratio, alpha, label) in fans {
                    ...
                    if use_gpu_drawing {
                        ...
                        {
                            use crate::chart::renderer_gpu::LineSegment;
                            let (s0, pv_slope) = gann_slot_val;
                            let fan_val_at = |slot: f32| *price0 + pv_slope * ratio * (slot - s0);
                            let (fan_dp, fan_dd) = match fan_ls {
                                LineStyle::Solid  => LineSegment::SOLID,
                                LineStyle::Dashed => LineSegment::DASHED,
                                LineStyle::Dotted => LineSegment::DOTTED,
                            };
                            chart.gpu_render_params.line_segments.push(LineSegment {
                                ...
                                end_slot:   edge_right_slot, end_val: fan_val_at(edge_right_slot),
                                color: _drawing_c32(color_alpha(dc, alpha)),
                                thickness: thick * drawing_ppp,
                                dash_period_px: fan_dp, dash_duty: fan_dd,
                            });
                        }
                    } else {
            ...
            DrawingKind::RegressionChannel{time0,time1} => {
                ...
                // The regression line and its σ bands are all collinear
                // (predicted = intercept + slope·x), so each reduces to a
                // single straight LineSegment / parallelogram FillQuad — no
                // per-bar polyline needed.
                ...
                // Endpoint values in price space.
                let pred_start = intercept as f32;
                let pred_end   = (intercept + slope * (n_reg - 1) as f64) as f32;
                if use_gpu_drawing {
                    #[cfg(feature = "gpu_chart_v2")]
                    {
                        use crate::chart::renderer_gpu::{LineSegment, FillQuad};
                        let s_a = start_idx as f32 - vs_floor;
                        let s_b = end_idx as f32 - vs_floor;
                        // 1σ shaded band (parallelogram between +σ and -σ).
                        chart.gpu_render_params.fill_quads.push(FillQuad {
                            start_slot: s_a, start_top: pred_start + sigma, start_bot: pred_start - sigma,
                            end_slot:   s_b, end_top:   pred_end + sigma,   end_bot:   pred_end - sigma,
                            color: _drawing_c32(hex_to_color(&d.color, d.opacity * 0.06)),
                        });
                        // Center regression line.
                        chart.gpu_render_params.line_segments.push(LineSegment {
                            start_slot: s_a, start_val: pred_start,
                            end_slot:   s_b, end_val:   pred_end,
                            color: _drawing_c32(if is_sel { t.accent } else { dc }),
                            thickness: (if is_sel { d.thickness + 1.0 } else { d.thickness }) * drawing_ppp,
                            dash_period_px: drawing_dash_period, dash_duty: drawing_dash_duty,
                        });
                        // ±σ and ±2σ band lines.
                        for &(sig, alpha) in &[(sigma, 120u8), (sigma * 2.0, 70u8)] {
                            let band_lc = _drawing_c32(color_alpha(dc, alpha));
                            let band_th = d.thickness * 0.6 * drawing_ppp;
                            chart.gpu_render_params.line_segments.push(LineSegment {
                                start_slot: s_a, start_val: pred_start + sig,
                                end_slot:   s_b, end_val:   pred_end + sig,
                                color: band_lc, thickness: band_th,
                                dash_period_px: drawing_dash_period, dash_duty: drawing_dash_duty,
                            });
                            chart.gpu_render_params.line_segments.push(LineSegment {
                                start_slot: s_a, start_val: pred_start - sig,
            ...
            DrawingKind::XABCD{points} if points.len() >= 2 => {
                ...
                if use_gpu_drawing {
                    ...
                    {
                        use crate::chart::renderer_gpu::LineSegment;
                        let lc = _drawing_c32(if is_sel { t.accent } else { dc });
                        let th = (if is_sel { d.thickness + 1.0 } else { d.thickness }) * drawing_ppp;
                        for i in 0..points.len().saturating_sub(1) {
                            if !(in_bounds(pts[i]) || in_bounds(pts[i+1])) { continue; }
                            let sa = SignalDrawing::time_to_bar(points[i].0,   &chart.timestamps) - vs_floor;
                            let sb = SignalDrawing::time_to_bar(points[i+1].0, &chart.timestamps) - vs_floor;
                            chart.gpu_render_params.line_segments.push(LineSegment {
                                start_slot: sa, start_val: points[i].1,
                                end_slot:   sb, end_val:   points[i+1].1,
                                color: lc, thickness: th,
                                dash_period_px: drawing_dash_period, dash_duty: drawing_dash_duty,
                            });
                        }
                ...
                if is_sel { for &pt in &pts { if in_bounds(pt) { painter.circle_stroke(pt, 6.0, egui::Stroke::new(1.5, t.text)); } } }
            }
            DrawingKind::ElliottWave{points,wave_type} if !points.is_empty() => {
                ...
                for (i, &pt) in pts.iter().enumerate() {
                    if in_bounds(pt) {
                        ...
                        let lbl = labels.get(i).copied().unwrap_or("?");
                        painter.text(pt, egui::Align2::CENTER_CENTER, lbl, mono_3xs(), t.text);
                    }
                }
                if is_sel { for &pt in &pts { if in_bounds(pt) { painter.circle_stroke(pt, 9.0, egui::Stroke::new(1.5, t.text)); } } }
            }
            DrawingKind::AnchoredVWAP{time} => {
                ...
                // VWAP is a curve — collect (slot, value) samples and push one
                // LineSegment per consecutive pair (same approach the indicator
                // line pipeline uses for SMA/EMA polylines).
                let mut pts = Vec::new();          // screen-space (egui fallback)
                let mut sv: Vec<(f32, f32)> = Vec::new(); // (slot, vwap) for GPU
                ...
                if use_gpu_drawing {
                    ...
                    {
                        use crate::chart::renderer_gpu::LineSegment;
                        let lc = _drawing_c32(if is_sel { t.accent } else { vwap_color });
                        let th = d.thickness * drawing_ppp;
                        for w in sv.windows(2) {
                            chart.gpu_render_params.line_segments.push(LineSegment {
                                start_slot: w[0].0, start_val: w[0].1,
                                end_slot:   w[1].0, end_val:   w[1].1,
                                color: lc, thickness: th,
                                dash_period_px: 0.0, dash_duty: 1.0,
                            });
                        }
                        // Faint anchor marker line (vertical).
                        let lo = chart.gpu_render_params.price_low;
                        let hi = chart.gpu_render_params.price_high;
                        let r = (hi - lo).max(1.0);
                        chart.gpu_render_params.line_segments.push(LineSegment {
                            start_slot: anchor_bar - vs_floor, start_val: hi + r * 2.0,
                            end_slot:   anchor_bar - vs_floor, end_val:   lo - r * 2.0,
                            color: _drawing_c32(color_alpha(vwap_color, 60)),
                            thickness: 0.5 * drawing_ppp,
                            dash_period_px: 0.0, dash_duty: 1.0,
                        });
                    }
                } else {
            ...
            DrawingKind::RiskReward{entry_price,entry_time,stop_price,target_price} => {
                ...
                if use_gpu_drawing {
                    ...
                    {
                        use crate::chart::renderer_gpu::{LineSegment, FillQuad};
                        let entry_bar = SignalDrawing::time_to_bar(*entry_time, &chart.timestamps);
                        let s_entry = entry_bar - vs_floor;
                        // Reward zone (entry→target) — green tint.
                        let rt_top = entry_price.max(*target_price);
                        let rt_bot = entry_price.min(*target_price);
                        chart.gpu_render_params.fill_quads.push(FillQuad {
                            start_slot: s_entry, start_top: rt_top, start_bot: rt_bot,
                            end_slot:   edge_right_slot, end_top: rt_top, end_bot: rt_bot,
                            color: _drawing_c32(color_alpha(COLOR_PROFIT_GREEN, 30)),
                        });
                        // Risk zone (entry→stop) — red tint.
                        let rk_top = entry_price.max(*stop_price);
                        let rk_bot = entry_price.min(*stop_price);
                        chart.gpu_render_params.fill_quads.push(FillQuad {
                            start_slot: s_entry, start_top: rk_top, start_bot: rk_bot,
                            end_slot:   edge_right_slot, end_top: rk_top, end_bot: rk_bot,
                            color: _drawing_c32(color_alpha(COLOR_LOSS_RED, 30)),
                        });
                        // Entry / stop / target lines, entry-bar → chart right.
                        let push_h = |segs: &mut Vec<LineSegment>, val: f32, col: egui::Color32, th: f32| {
                            segs.push(LineSegment {
                                start_slot: s_entry, start_val: val,
                                end_slot:   edge_right_slot, end_val: val,
                                color: _drawing_c32(col), thickness: th * drawing_ppp,
                                dash_period_px: 0.0, dash_duty: 1.0,
                            });
                        };
                        let segs = &mut chart.gpu_render_params.line_segments;
                        push_h(segs, *entry_price,  color_alpha(dc, 200), d.thickness);
                        push_h(segs, *stop_price,   COLOR_LOSS_RED,       d.thickness * 0.8);
                        push_h(segs, *target_price, COLOR_PROFIT_GREEN,   d.thickness * 0.8);
                    }
                } else {
                    painter.rect_filled(egui::Rect::from_min_max(egui::pos2(ex, ey.min(ty)), egui::pos2(chart_right, ey.max(ty))),
                        0.0, color_alpha(COLOR_PROFIT_GREEN, 30));
                    painter.rect_filled(egui::Rect::from_min_max(egui::pos2(ex, ey.min(sy)), egui::pos2(chart_right, ey.max(sy))),
                        0.0, color_alpha(COLOR_LOSS_RED, 30));
            ...
            DrawingKind::FibTimeZone{time} => {
                ...
                let chart_right = rect.left() + cw;
                // Vertical-line price span (2× beyond visible range; scissor clips).
                #[cfg(feature = "gpu_chart_v2")]
                let (vz_top, vz_bot) = {
                    let lo = chart.gpu_render_params.price_low;
                    let hi = chart.gpu_render_params.price_high;
                ...
                for (idx, &fib) in fib_nums.iter().enumerate() {
                    ...
                    if use_gpu_drawing {
                        ...
                        {
                            use crate::chart::renderer_gpu::LineSegment;
                            let slot = target_bar - vs_floor;
                            let (dp, dd) = LineSegment::DASHED;
                            chart.gpu_render_params.line_segments.push(LineSegment {
                                start_slot: slot, start_val: vz_top,
                                end_slot:   slot, end_val:   vz_bot,
                                color: _drawing_c32(color_alpha(dc, alpha)),
                                thickness: d.thickness * 0.7 * drawing_ppp,
                                dash_period_px: dp, dash_duty: dd,
                            });
                        }
                    } else {
                ...
                if ax.is_finite() && ax >= rect.left() && ax <= chart_right {
                    if use_gpu_drawing {
                        ...
                        {
                            use crate::chart::renderer_gpu::LineSegment;
                            let slot = anchor_bar - vs_floor;
                            chart.gpu_render_params.line_segments.push(LineSegment {
                                start_slot: slot, start_val: vz_top,
                                end_slot:   slot, end_val:   vz_bot,
                                color: _drawing_c32(color_alpha(dc, 200)),
                                thickness: d.thickness * drawing_ppp,
                                dash_period_px: 0.0, dash_duty: 1.0,
                            });
                        }
                    } else {
            ...
            DrawingKind::TextNote { price, time, text, font_size } => {
                ...
                if x.is_finite() && y.is_finite() && x.abs() < 50000.0 && y.abs() < 50000.0 {
                    painter.text(egui::pos2(x, y), egui::Align2::LEFT_TOP, text,
                        prop_at(*font_size), dc);
                    if is_sel {
                        let galley = painter.layout_no_wrap(text.clone(), prop_at(*font_size), dc);
    ...
    if chart.show_divergences && !chart.divergence_markers.is_empty() {
        for dm in &chart.divergence_markers {
            ...
            let line_color = color_alpha(color, alpha);
    ...
    if chart.show_pattern_labels && !chart.pattern_labels.is_empty() {
        render_pattern_labels_overlay(&painter, chart, rect, cw, t, &py, &bx);
    }
    ...
    if chart.show_signal_zones { for zone in &chart.signal_zones {
        ...
        let zone_color = match zone.zone_type.as_str() {
            ...
            // Unknown zone kind: neutral, so it reads as "no signal".
            _ => t.dim,
        };
    ...
    if chart.show_vol_shelves && !chart.bars.is_empty() {
        ...
        for (i, &v) in vol.iter().enumerate() {
            ...
            let color = if is_support {
                color_alpha(t.bull, (strength * 25.0) as u8)
            } else {
                color_alpha(t.bear, (strength * 25.0) as u8)
            };
    ...
    if chart.show_vol_cone && chart.bars.len() > 20 {
        ...
        for &sigma in &[1.0f32, 2.0, 3.0] {
            ...
            let col = color_alpha(t.accent, alpha);
            ...
            if points_upper.len() >= 2 {
                ...
                let line_col = color_alpha(t.accent, line_alpha);
    ...
    if chart.show_price_memory && chart.bars.len() > 20 {
        ...
        for (i, &count) in touches.iter().enumerate() {
            ...
            painter.rect_filled(egui::Rect::from_min_max(
                ...
                0.0, color_alpha(t.accent, alpha));
        }
    }
    ...
    if chart.show_liquidity_voids && chart.bars.len() > 5 {
        ...
        for i in (vis_start + 1)..vis_end {
            ...
            let border_col = if is_up {
                color_alpha(t.bull, 40)
            } else {
                color_alpha(t.bear, 40)
            };
    ...
    for order in &chart.orders {
        ...
        if let Some(trail_amt) = order.trail_amount {
            ...
            if trail_y.is_finite() && trail_y >= rect.top() + pt && trail_y <= rect.top() + pt + ch {
                let trail_color = color_alpha(t.bear, 60);
                let mut dx = rect.left();
                while dx < rect.left() + cw {
        ...
        if let Some(pos) = ui.input(|i| i.pointer.hover_pos()) {
            ...
            if !chart.bars.is_empty() { if let Some(bar_data) = chart.bars.get(bar_idx) {
                ...
                if best_dist < magnet_radius {
                    ...
                    painter.circle_stroke(egui::pos2(bar_x, sy), 4.5, egui::Stroke::new(1.0, contrast_fg(t.accent)));
    ...
    if pointer_in_pane && chart.draw_tool.is_empty() {
            ...
            if pos.x >= rect.left() && pos.x < rect.left()+cw && pos.y >= rect.top()+pt && pos.y < rect.top()+pt+ch {
                ...
                if chart.measure_tooltip && last_price > 0.0 {
                    ...
                    painter.text(egui::pos2(measure_rect.center().x, measure_rect.top() + pct_galley.size().y / 2.0 + 4.0),
                        egui::Align2::CENTER_CENTER, &m_pct_text, mono_md(), dist_col);
                    painter.text(egui::pos2(measure_rect.center().x, measure_rect.bottom() - price_galley.size().y / 2.0 - 3.0),
                        egui::Align2::CENTER_CENTER, &m_price_text, mono_sm(), color_alpha(dist_col, 160));
                }
                ...
                if chart.ohlc_tooltip && !chart.show_footprint && !chart.draw_picker.open {
                    if let Some(bar_data) = chart.bars.get(bar_idx) {
                        ...
                        for ind in &chart.indicators {
                            ...
                            let col = color_alpha(ind_color, alpha);
                            match ind.kind {
                                IndicatorType::MACD => {
        ...
        render_footprint_overlay(&painter, chart, rect, ch, pt, t, bar_idx, bs, &py, &bx);
    }
            }
        }
    }
    ...
    if chart.show_pnl_curve {
        render_pnl_curve_overlay(&painter, chart, rect, cw, ch, pt, t, account_data_cached);
    }
    ...
    // ── PRIORITY 0: Strikes overlay button — moved to pane header right cluster.
    //    See `hdr.clicked_options` handler earlier in this file.
            ...
            if chart.measure.active {
                ...
                if let Some((sb, sp)) = chart.measure.start {
                    ...
                    let fill_color = if price_diff >= 0.0 {
                        color_alpha(t.bull, 20)
                    } else {
                        color_alpha(t.bear, 20)
                    };
                    ...
                    painter.rect_filled(label_rect, 3.0, color_alpha(t.toolbar_bg, 220));
    ...
    if pane_idx == *active_pane {
        super::keyboard_shortcuts::handle_keyboard_shortcuts(ui, ctx, chart, watchlist);
    }
    ...
    if !chart.show_gamma || chart.gamma_levels.is_empty() { return; }
        ...
        if chart.gamma_synthetic {
            ...
            painter.rect_filled(egui::Rect::from_min_size(
                ...
                4.0, color_alpha(t.toolbar_bg, 220));
            ...
            if chart.gamma_flow_active {
                if let Some(ppe) = chart.gamma_ppe {
                    ...
                    let ppe_col = if ppe >= 1.20 {
                        ...
                        // Neither extreme: neutral/no-signal.
                        t.dim
                    };
        ...
        if cw_y.is_finite() && cw_y > rect.top() + pt && cw_y < rect.top() + pt + ch {
            ...
            painter.rect_filled(egui::Rect::from_min_size(egui::pos2(lx - 6.0, cw_y - galley.size().y / 2.0 - 3.0), galley.size() + egui::vec2(12.0, 6.0)),
                4.0, color_alpha(t.toolbar_bg, 230));
...
fn render_play_lines_overlay(
    ...
    if chart.play_lines.is_empty() { return; }
        ...
        if let (Some(tp), Some(sp)) = (target_price, stop_price) {
            ...
            if let Some(ep) = entry_price {
                ...
                if risk > 0.0 {
                    ...
                    painter.rect_filled(rr_bg, 3.0, color_alpha(t.toolbar_bg, 220));
        ...
        for pl in &chart.play_lines {
            ...
            let dash_color = color_alpha(line_color, 150);
            let mut dx = rect.left();
            while dx < rect.left() + cw {
                let end = (dx + 8.0).min(rect.left() + cw);
            ...
            painter.rect_filled(kind_rect, egui::CornerRadius { nw: 3, sw: 3, ne: 0, se: 0 },
                color_alpha(line_color, 220u8.saturating_add(hb)));
            ...
            painter.rect_filled(price_rect, 0.0,
                color_alpha(line_color, 180u8.saturating_add(hb)));
            ...
            painter.rect_filled(play_rect, egui::CornerRadius { nw: 0, sw: 0, ne: 3, se: 3 },
                color_alpha(line_color, 140u8.saturating_add(hb)));
                        ...
                        if is_oco {
                            ...
                            if risk > 0.0 {
                                ...
                                let rr_bg_rect = egui::Rect::from_min_size(
                                    ...
                                    egui::vec2(rr_galley.size().x + 6.0, rr_galley.size().y + 4.0));
                                painter.rect_filled(rr_bg_rect, 3.0, color_alpha(t.toolbar_bg, 220));
                            ...
                            let band_color = match order.side {
                                OrderSide::TriggerBuy | OrderSide::TriggerSell => color_alpha(t.bull, 12),
                            ...
                            let connector_x = rect.left() + cw - 20.0;
                            {
                                ...
                                while dy < y1.max(y2) {
                                    ...
                                    painter.line_segment(
                                        [egui::pos2(connector_x, dy), egui::pos2(connector_x, end)],
                                        egui::Stroke::new(0.8, color_alpha(t.bull, 80)));
                                    dy += 6.0;
                                }
                            }
                        }
...
fn render_price_alert_lines(
    ...
    ui: &mut egui::Ui, py: impl Fn(f32) -> f32,
        ...
        for &aid in &alert_ids {
            ...
            let dash_col = color_alpha(alert_color, if is_hovered || is_dragging { 255 } else { base_alpha });
            let mut dx = rect.left();
            ...
            // Drag handle pill at center of line — visual grab target
            {
                ...
                let dot_col = color_alpha(contrast_fg(alert_color), 180);
            ...
            let label_font = mono_at(if is_draft { font_xs() } else { font_2xs() });
            ...
            if is_draft {
                ...
                painter.text(egui::pos2(lx + pad, y), egui::Align2::LEFT_CENTER, &label_text, label_font, contrast_fg(alert_color));
                ...
                let on_alert = contrast_fg(alert_color);
                let place_bg = if place_hover { on_alert } else { color_alpha(on_alert, 230) };
                ...
                painter.text(x_rect.center(), egui::Align2::CENTER_CENTER, "\u{00D7}",
                    mono_md_plus(), contrast_fg(alert_color));
                ...
                painter.rect_filled(badge_rect, 3.0, color_alpha(t.toolbar_bg, 220));
    ...
    if !(chart.show_darkpool && !chart.darkpool_prints.is_empty() && !chart.bars.is_empty() && !chart.timestamps.is_empty()) { return; }
        ...
        for dp_print in &chart.darkpool_prints {
            ...
            let (fill_col, stroke_col) = match dp_print.side {
                1 => (
                    ...
                    color_alpha(t.bull, 160),
                ),
                -1 => (
                    color_alpha(t.bear, 80),
                    color_alpha(t.bear, 160),
                ),
                _ => (
                    color_alpha(t.dim, 60),
                    color_alpha(t.dim, 100),
                ),
            };
        ...
        for (_key, (level_price, total_vol)) in &price_volume {
            ...
            painter.rect_filled(egui::Rect::from_min_size(
                ...
                3.0, color_alpha(t.toolbar_bg, 220));
...
fn render_events_overlay(
    painter: &egui::Painter, chart: &Chart, rect: egui::Rect, cw: f32, ch: f32, pt: f32, t: &Theme, ui: &mut egui::Ui, bx: impl Fn(f32) -> f32
        ...
        for em in &chart.event_markers {
            ...
            if is_earnings {
                ...
                painter.text(egui::pos2(x, marker_y + sq_sz + 3.0), egui::Align2::CENTER_TOP,
                    arrow_icon, prop_3xs(), impact_col);
            } else {
        ...
        if let Some((pos, label, details, col)) = hovered_tooltip {
            ...
            painter.rect_filled(tip_rect, 4.0, color_alpha(t.toolbar_bg, 235));
...
fn render_prev_close_overlay(
    painter: &egui::Painter, chart: &Chart, rect: egui::Rect, cw: f32, t: &Theme, py: impl Fn(f32) -> f32
        ...
        if let Some(pc) = prev_close {
            ...
            if y.is_finite() {
                dashed_line(&painter, egui::pos2(rect.left(), y), egui::pos2(rect.left()+cw, y),
                    egui::Stroke::new(0.5, color_alpha(t.dim, 80)), LineStyle::Dashed);
                painter.text(egui::pos2(rect.left()+cw+3.0, y), egui::Align2::LEFT_CENTER,
...
/// render_pnl_curve_overlay (WS-E E4: extracted from render_chart_pane). Verbatim; guard at call site.
#[allow(clippy::too_many_arguments)]
fn render_pnl_curve_overlay(
    painter: &egui::Painter, chart: &Chart, rect: egui::Rect, cw: f32, ch: f32, pt: f32, t: &Theme,
    account_data_cached: &Option<(AccountSummary, Vec<Position>, Vec<IbOrder>)>,
) {
        let pnl_h = 60.0_f32;
        ...
        // Subtle background
        painter.rect_filled(pnl_rect, 0.0, color_alpha(t.bg, 180));
        painter.line_segment(
...
/// render_footprint_overlay (WS-E E4: extracted from render_chart_pane). Verbatim; guard at call site.
#[allow(clippy::too_many_arguments)]
fn render_footprint_overlay(
    painter: &egui::Painter, chart: &Chart, rect: egui::Rect, ch: f32, pt: f32, t: &Theme, bar_idx: usize, bs: f32, py: impl Fn(f32) -> f32, bx: impl Fn(f32) -> f32
) {
                    if let Some(bar_data) = chart.bars.get(bar_idx) {
                        let bar_x = bx(bar_idx as f32);
                        ...
                        if bar_range > 0.0 && bar_data.volume > 0.0 {
                            ...
                            for (li, info) in fp_levels.iter().enumerate() {
                                ...
                                painter.rect_filled(card_rect, 6.0, color_alpha(t.toolbar_bg, 245));
                                ...
                                painter.text(egui::pos2(cx, cy + 13.0), egui::Align2::LEFT_CENTER,
                                    &format!("{:.2}", info.price), font_price.clone(), t.text);
...
/// render_pattern_labels_overlay (WS-E E4: extracted from render_chart_pane). Verbatim; guard at call site.
#[allow(clippy::too_many_arguments)]
fn render_pattern_labels_overlay(
    painter: &egui::Painter, chart: &Chart, rect: egui::Rect, cw: f32, t: &Theme, py: impl Fn(f32) -> f32, bx: impl Fn(f32) -> f32
) {
        let bars_ref = if chart.candle_mode == CandleMode::Standard { &chart.bars } else { &chart.alt.bars };
        let ts_ref = if chart.candle_mode == CandleMode::Standard { &chart.timestamps } else { &chart.alt.timestamps };
...
/// render_cvd_overlay (WS-E E4: extracted from render_chart_pane). Verbatim; guard at call site.
#[allow(clippy::too_many_arguments)]
fn render_cvd_overlay(
    painter: &egui::Painter, chart: &Chart, rect: egui::Rect, cw: f32, t: &Theme, vs: f32, osc_top: f32, osc_bottom: f32, n: usize, bx: impl Fn(f32) -> f32
) {
...
fn render_order_lines(
    ...
    for order in &chart.orders {
        ...
        let dash_color = color_alpha(line_color, dash_alpha);
        ...
        let side_bg = color_alpha(color, badge_alpha.saturating_add(20).saturating_add(hover_boost));
        ...
        painter.rect_filled(qty_rect, 0.0, color_alpha(color, badge_alpha.saturating_add(hover_boost)));
        ...
        painter.rect_filled(not_rect, 0.0, color_alpha(color, badge_alpha.saturating_sub(10).saturating_add(hover_boost)));
        ...
        painter.rect_filled(status_rect, 0.0, color_alpha(color, badge_alpha.saturating_sub(40).saturating_add(hover_boost)));
        ...
        if is_draft {
            ...
            let send_bg = if send_hovered {
                ...
                color_alpha(t.accent, 120)
            };
            painter.rect_filled(send_rect, 0.0, send_bg);
            painter.text(send_rect.center(), egui::Align2::CENTER_CENTER, "SEND", mono_2xs(), contrast_fg(t.accent));
...
fn render_trigger_levels(
    ...
    for tl in &chart.trigger_levels {
        ...
        painter.text(tag_rect.center(), egui::Align2::CENTER_CENTER,
            &format!("{:.2}", tl.trigger_price), mono_xs(), contrast_fg(color));
    }
}
...
fn render_change_points_overlay(
    ...
    let bars_ref = if chart.candle_mode == CandleMode::Standard { &chart.bars } else { &chart.alt.bars };
    let ts_ref = if chart.candle_mode == CandleMode::Standard { &chart.timestamps } else { &chart.alt.timestamps };
    // No `&Theme` reaches this extracted helper — take the ambient one.
    let t_dim = crate::chart_renderer::theme_impl::active_theme(painter.ctx()).dim;
...
fn render_hit_highlight_overlay(
    painter: &egui::Painter, chart: &mut Chart, rect: egui::Rect, cw: f32, t: &Theme, vs: f32, n: usize, ctx: &egui::Context, end: u32, py: impl Fn(f32) -> f32, bx: impl Fn(f32) -> f32
        ...
        for &(key, ref hit_time) in &chart.hit_highlights {
            ...
            if alpha < 5 { continue; }
            let flash_color = color_alpha(t.text, alpha);
