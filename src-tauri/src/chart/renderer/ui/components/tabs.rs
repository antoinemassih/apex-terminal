    ...
    pub fn show(self, ui: &mut Ui) {
        ...
        for (tab, label) in self.tabs {
            ...
            let btn = egui::Button::new(
                SemanticLabel::new(*label, LabelVariant::Tab).size(fs).color(color).into_rich_text()
        ...
        ui.horizontal(|ui| {
            ...
            for (i, label) in self.tabs.iter().enumerate() {
                ...
                if is_active && !st.hairline_borders {
                    ...
                    let resp = ui.add(
                        egui::Button::new(
                            SemanticLabel::new(text, LabelVariant::Tab).size(font_md()).color(fg).into_rich_text(),
                        )
            ...
            for (i, label) in self.tabs.iter().enumerate() {
                ...
                ui.horizontal(|ui| {
                    ...
                    if is_active && !st.hairline_borders {
                        let resp = ui.add(
                            egui::Button::new(
                                SemanticLabel::new(s, LabelVariant::Tab).color(fg).into_rich_text(),
                            )
                            .fill(color_alpha(self.accent, alpha_tint()))
                            .stroke(Stroke::NONE)
                            .corner_radius(r_pill())
