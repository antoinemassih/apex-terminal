...
pub(crate) struct Theme {
    ...
    // ── M1 Change A/C: authored ramp + bevel tints (None = derive) ──────────
    // Carried from `ColorScheme` by `color_scheme_to_theme` so the
    // ComponentTheme impl can prefer authored surfaces over `elevate()`.
    pub(crate) bg_panel:        Option<egui::Color32>,
    pub(crate) bg_elevated:     Option<egui::Color32>,
    pub(crate) bg_hover:        Option<egui::Color32>,
    pub(crate) fg_xmuted:       Option<egui::Color32>,
    pub(crate) accent_sub:      Option<egui::Color32>,
    pub(crate) bull_alpha:      Option<egui::Color32>,
    pub(crate) bear_alpha:      Option<egui::Color32>,
    pub(crate) border_dim:      Option<egui::Color32>,
    pub(crate) bevel_highlight: Option<egui::Color32>,
    pub(crate) bevel_shadow:    Option<egui::Color32>,
        ...
        | W::OptionsSentiment | W::Volatility => {
            ...
            for i in 0..16 {
                ...
                p.line_segment([
                    ...
                    egui::Stroke::new(style::stroke_rule(), col));
            }
        }
        // Concentric rings
...
impl ScannerDef {
    pub(crate) fn preset_gainers() -> Self {
    ...
    pub(crate) fn preset_losers() -> Self {
    ...
    pub(crate) fn preset_most_active() -> Self {
...
// ── Watchlist sub-context state types moved to watchlist_state.rs (WS-E E3).
//    Re-export so `Watchlist`'s field types + any external `gpu::*State` refs
//    resolve unchanged.
pub(crate) use super::watchlist_state::*;
...
impl GpuCtx {
    fn new(window: Arc<Window>) -> Option<Self> {
        ...
        // AT-076: DUAL_SOURCE_BLENDING was requested solely for the subpixel-AA
        // text pipeline, which is gone. Requesting a device feature nothing uses
        // narrows the set of adapters that can satisfy the request for no
        // benefit, so it goes with it. Text renders through cosmic-text +
        // swash grayscale AA, which is what the one production call site was
        // already falling back to on any adapter without this feature.
