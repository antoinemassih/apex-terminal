    ...
    for (ci, (oid, _created)) in c.chart.pending_confirms.iter().enumerate() {
        ...
        if let Some((label, price, qty, color)) = order_data {
            ...
            egui::Window::new(format!("confirm_toast_{}_{}", c.pane_idx, oid))
                ...
                .title_bar(false)
                .frame(egui::Frame::popup(&c.ctx.style()).fill(c.t.toolbar_bg).inner_margin(4.0)
                    .stroke(egui::Stroke::new(1.0, color)))
                .show(c.ctx, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(egui::RichText::new(format!("{} x{} @ {:.2}", label, qty, price)).monospace().size(9.0).color(color));
                        if ui.add(egui::Button::new(egui::RichText::new(Icon::CHECK).size(12.0).color(c.t.bull))
                        ...
                        if ui.add(egui::Button::new(egui::RichText::new(Icon::X).size(9.0).color(c.t.bear))
                            .corner_radius(2.0).min_size(egui::vec2(24.0, 20.0))).clicked() {
                            cancel_ids.push(*oid);
                        }
                    });
