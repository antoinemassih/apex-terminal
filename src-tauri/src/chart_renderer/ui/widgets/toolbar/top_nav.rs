...
pub(crate) fn render(
    ...
    egui::TopBottomPanel::top("tb")
        ...
        ui.horizontal_centered(|ui| {
            ...
            // ── Paper / Live — colored text, no background fill ──
            {
                ...
                let resp = ui.add(super::super::buttons::ChromeBtn::new(
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                if ui.add(ToolbarBtn::new(&format!("{} Window", Icon::PLUS)).active(false).theme(t)).clicked() {
                    ...
                    crate::chart_renderer::gpu::fetch_bars_background(panes[ap].symbol.clone(), panes[ap].timeframe.clone());
                }
    ...
    if watchlist.account_strip_open {
        ...
        egui::TopBottomPanel::top("account_strip")
            ...
            .show(ctx, |ui| {
                super::super::pane::AccountStrip::new()
    ...
    // ── hotkey_editor
    crate::chart_renderer::ui::hotkey_editor::draw(ctx, watchlist, panes, ap, t);

    // ── Settings panel
    crate::chart_renderer::ui::settings_panel::draw(ctx, watchlist, &mut panes[ap], t, ap);
    crate::chart_renderer::ui::apex_diagnostics::draw(ctx, watchlist, t);

    // ── trendline_filter
    crate::chart_renderer::ui::trendline_filter::draw(ctx, watchlist, panes, ap, t);
    crate::chart_renderer::ui::option_quick_picker::draw(ctx, watchlist, panes, ap, t);
    crate::chart_renderer::ui::template_popup::draw(ctx, watchlist, panes, ap, t);

    // ── indicator_editor
    crate::chart_renderer::ui::indicator_editor::draw(ctx, watchlist, panes, ap, t);

    // ── overlay_manager
    crate::chart_renderer::ui::overlay_manager::draw(ctx, watchlist, panes, ap, t);
    ...
    // ── connection_panel
    crate::chart_renderer::ui::connection_panel::draw(ctx, watchlist, panes, ap, t, conn_panel_open);
    ...
    crate::chart_renderer::ui::watchlist_panel::draw(ctx, watchlist, panes, ap, t);

    // ── Object Tree side panel
    crate::chart_renderer::ui::object_tree::draw(ctx, watchlist, panes, ap, t);

    // ── Book pane (Positions/Orders + Journal tabs) ─────────────────────────
    crate::chart_renderer::ui::orders_panel::draw(ctx, watchlist, panes, ap, t, account_data_cached);

    // ── Scanner side panel
    crate::chart_renderer::ui::scanner_panel::draw(ctx, watchlist, panes, ap, t);

    // ── Time & Sales side panel
    crate::chart_renderer::ui::tape_panel::draw(ctx, watchlist, &panes[ap].symbol, t);

    // ── RRG (Relative Rotation Graph) side panel
    crate::chart_renderer::ui::rrg_panel::draw(ctx, watchlist, t);

    // ── Analysis sidebar (unified RRG / T&S / Scanner / Scripts)
    crate::chart_renderer::ui::analysis_panel::draw(ctx, watchlist, panes, *active_pane, t);

    // ── Signals sidebar (unified Alerts + Signals)
    crate::chart_renderer::ui::signals_panel::draw(ctx, watchlist, panes, ap, t);

    // ── Feed sidebar (unified News + Discord + Screenshots)
    crate::chart_renderer::ui::feed_panel::draw(ctx, watchlist, panes, ap, t);

    // ── Playbook sidebar
    crate::chart_renderer::ui::playbook_panel::draw(ctx, watchlist, panes, ap, t);

    // ── Journal sidebar
    crate::chart_renderer::ui::journal_panel::draw(ctx, watchlist, t);

    // ── Script / Backtesting panel
    crate::chart_renderer::ui::script_panel::draw(ctx, watchlist, t);

    // ── Spread Builder panel
    crate::chart_renderer::ui::spread_panel::draw(ctx, watchlist, &panes[ap].symbol, t);
