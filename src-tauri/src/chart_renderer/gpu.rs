...
const CHART_RIGHT_PAD: u32 = 20;           // Empty bars of space to the right of latest bar
...
fn route_commands(rx: &mpsc::Receiver<ChartCommand>, panes: &mut [Chart], active_pane: &mut usize, watchlist: &mut Watchlist) {
...
fn check_history_pagination(panes: &mut [Chart], active_pane: usize) {
...
fn update_simulation(panes: &mut [Chart]) {
    use crate::monitoring::{span_begin, span_end};
...
fn render_toolbar(
    ...
    // ── overlay_manager
    super::ui::overlay_manager::draw(ctx, watchlist, panes, ap, t);
    ...
    // ── connection_panel
    super::ui::connection_panel::draw(ctx, watchlist, panes, ap, t, conn_panel_open);
    ...
    super::ui::watchlist_panel::draw(ctx, watchlist, panes, ap, t);
...
fn render_chart_pane(
    ...
    if show_header {
        ...
        if has_tabs {
            // Render browser-style tab bar
            let tab_h = pane_top_offset - 2.0; // tab height within header
            ...
            let tab_data: Vec<(String, String, f32)> = (0..tab_count).map(|i| {
                (chart.tab_symbols[i].clone(), chart.tab_timeframes[i].clone(),
            ...
            for (ti, (sym, _tf, chg)) in tab_data.iter().enumerate() {
                let is_active_tab = ti == active_tab;
                // Measure tab text width
                let chg_text = if *chg >= 0.0 { format!("+{:.1}%", chg) } else { format!("{:.1}%", chg) };
                let sym_galley = header_painter.layout_no_wrap(sym.clone(), egui::FontId::monospace(9.0), t.dim);
                let chg_galley = header_painter.layout_no_wrap(chg_text.clone(), egui::FontId::monospace(8.0), t.dim);
                let close_w = 12.0;
                let tab_pad = 8.0;
                let gap = 4.0;
                let tab_w = tab_pad + sym_galley.size().x + gap + chg_galley.size().x + gap + close_w + tab_pad;
                ...
                // Symbol text
                let sym_col = if is_active_tab { egui::Color32::from_rgb(220, 220, 230) } else { t.dim.gamma_multiply(0.7) };
                header_painter.text(
                    egui::pos2(tab_x + tab_pad, tab_rect.center().y),
                    egui::Align2::LEFT_CENTER, sym, egui::FontId::monospace(9.0), sym_col);
                // Change % badge
                let chg_col = if *chg >= 0.0 { t.bull } else { t.bear };
                let chg_alpha = if is_active_tab { 255u8 } else { 140 };
                let chg_draw_col = egui::Color32::from_rgba_unmultiplied(chg_col.r(), chg_col.g(), chg_col.b(), chg_alpha);
                let chg_x = tab_x + tab_pad + sym_galley.size().x + gap;
                header_painter.text(
                    egui::pos2(chg_x, tab_rect.center().y),
                    egui::Align2::LEFT_CENTER, &chg_text, egui::FontId::monospace(8.0), chg_draw_col);
                ...
                if show_close {
                    ...
                    let close_col = if close_resp.hovered() { t.bear } else { t.dim.gamma_multiply(0.5) };
                    if close_resp.hovered() { ui.ctx().set_cursor_icon(egui::CursorIcon::PointingHand); }
                    if close_resp.hovered() {
                        header_painter.rect_filled(close_rect, 2.0, color_alpha(t.bear, 20));
                    }
                    header_painter.text(close_rect.center(), egui::Align2::CENTER_CENTER,
                        "\u{00D7}", egui::FontId::proportional(11.0), close_col);
            ...
            {
                let plus_rect = egui::Rect::from_min_size(egui::pos2(tab_x + 2.0, tab_y), egui::vec2(20.0, tab_h));
                let plus_resp = ui.allocate_rect(plus_rect, egui::Sense::click());
                let plus_col = if plus_resp.hovered() { t.dim } else { t.dim.gamma_multiply(0.5) };
                if plus_resp.hovered() {
                    ui.ctx().set_cursor_icon(egui::CursorIcon::PointingHand);
                    header_painter.rect_filled(plus_rect, 3.0, color_alpha(t.toolbar_border, 20));
                }
                header_painter.text(plus_rect.center(), egui::Align2::CENTER_CENTER,
                    "+", egui::FontId::monospace(12.0), plus_col);
                if plus_resp.clicked() {
                    // Add a new tab with the current symbol (user can change it via picker)
            ...
            // OVL button after tab bar
            {
                let sym_text_w = 0.0; // not used in tab mode, OVL goes after tabs
                let _ = sym_text_w;
                let ovl_x = tab_x + 26.0;
    ...
    // ── Floating order panes (reuse shared order entry component) ─────
    {
        ...
        for pane in &mut floating_panes {
            let adv = chart.order_advanced;
            let fp_panel_w = if adv { 300.0 } else { 230.0 };
    ...
    if watchlist.order_entry_open {
        ...
        // Window stays at abs_pos — DOM grows downward between header and body
        ...
        if false { // old separate DOM window — removed
            ...
            egui::Window::new(format!("dom_{}", pane_idx))
                ...
                .show(ctx, |ui| {
                    ...
                    let current_price = chart.bars.last().map(|b| b.close).unwrap_or(100.0);
                    let is_index = chart.symbol == "SPX" || chart.symbol == "NDX" || chart.symbol == "DJI" || chart.symbol == "RUT";
                    let tick = if is_index { 1.0 } else { 0.01 };
                    let rows_above = 20;
                    let rows_below = 20;
                    // Snap current price to tick grid
                    let center_price = (current_price / tick).round() * tick;

                    // Simulated bid/ask sizes (deterministic pseudo-random based on price level)
                    let sim_size = |price: f32, is_bid: bool| -> u32 {
                        let dist = ((price - current_price).abs() / tick).round() as u32;
                        let base = 50u32.saturating_sub(dist * 2).max(1);
                        let hash = ((price * 1000.0) as u32).wrapping_mul(2654435761);
                        let extra = (hash % 100) as u32;
                        let size = base + extra;
                        if is_bid { size } else { size + 20 }
                    };

                    // Find user's position entry price
                    let position_entry = account_data_cached.as_ref()
                        .and_then(|(_, positions, _)| positions.iter().find(|p| p.symbol == chart.symbol))
                        .map(|p| p.avg_price);
                    ...
                    egui::ScrollArea::vertical().max_height(dom_h - 40.0).auto_shrink([false, false]).show(ui, |ui| {
                        for row in (-rows_above..=rows_below).rev() {
                            ...
                            let row_bg = if is_current {
                                color_alpha(t.accent, 35)
                            } else {
                                egui::Color32::TRANSPARENT
                            };
                            // Highlight: user's open orders on matching price
                            let has_buy_order = chart.orders.iter().any(|o| (o.price - price).abs() < tick * 0.5 && matches!(o.side, OrderSide::Buy));
                            let has_sell_order = chart.orders.iter().any(|o| (o.price - price).abs() < tick * 0.5 && matches!(o.side, OrderSide::Sell));
                            ...
                            let row_rect = ui.horizontal(|ui| {
                                ...
                                if has_sell_order {
                                    ui.painter().rect_filled(row_r, 0.0, color_alpha(t.bear, 25));
                                }
                                if is_entry {
...
fn draw_chart(ctx: &egui::Context, panes: &mut Vec<Chart>, active_pane: &mut usize, layout: &mut Layout, watchlist: &mut Watchlist, toasts: &[(String, f32, std::time::Instant, bool)], conn_panel_open: &mut bool, rx: &mpsc::Receiver<ChartCommand>) {
    ...
    egui::CentralPanel::default().frame(egui::Frame::NONE.fill(t.bg)).show(ctx, |ui| {
        ...
        let pane_rects = layout.pane_rects(full_rect, visible_count, watchlist.pane_split_h, watchlist.pane_split_v, watchlist.pane_split_h2, watchlist.pane_split_v2);
