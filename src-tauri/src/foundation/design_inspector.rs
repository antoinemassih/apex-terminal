...
let _ = ctx;
            ...
            for h in &hits {
                ...
                // TODO(design-tokens): status color tokens — purple (203,166,247), green (166,227,161),
                // peach (250,179,135), sky (137,220,235), pink (243,139,168) are semantic inspector
                // colors with no token yet. Add status_purple/status_green/etc. tokens when available.
    ...
    fn render_category(&mut self, ui: &mut Ui, cat: Category, tokens: &mut DesignTokens) -> bool {
        ...
        match cat {
            ...
            Category::Alpha => {
                ...
                changed |= drag_u8(ui, "whisper (25)", &mut tokens.alpha.whisper);
                changed |= drag_u8(ui, "hint (30)", &mut tokens.alpha.hint);
                changed |= drag_u8(ui, "subtle (40)", &mut tokens.alpha.subtle);
                changed |= drag_u8(ui, "tint (48)", &mut tokens.alpha.tint);
                changed |= drag_u8(ui, "muted (40)", &mut tokens.alpha.muted);
                changed |= drag_u8(ui, "line (80)", &mut tokens.alpha.line);
                ...
                changed |= drag_u8(ui, "dense (160)", &mut tokens.alpha.dense);
                changed |= drag_u8(ui, "near_solid (180)", &mut tokens.alpha.near_solid);
